...
class Car:
    ...
    def initImgs(self):
        ...
        self.img = py.transform.rotate(
            py.transform.scale(
                py.image.load(os.path.join(assets_dir, name)).convert_alpha(), (120, 69)
            ),
            -90,
        )
        self.brake_img = py.transform.rotate(
            py.transform.scale(
                py.image.load(os.path.join(assets_dir, "brakes.png")).convert_alpha(),
                (120, 69),
            ),
            -90,
        )
            ...
            for k, s in enumerate(sensors):
                ...
                if s < SENSOR_DISTANCE:
                    py.draw.circle(
                        world.win,
                        RED,
                        world.getScreenCoords(self.x + dx, self.y + dy),
                        6,
                    )
